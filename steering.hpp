...
int scanPatterns() {
	...
	uintptr_t steering_control = patterns::scan(gta5, "F3 44 0F 11 87 ? ? ? ? F6 80 ? ? ? ? ? 74 4D");
	uintptr_t steering_parked = patterns::scan(gta5, "44 89 BB ? ? ? ? 8B 0D ? ? ? ? 0F 57 C0 81 F9 ? ? ? ? 74 ?");
	...
	if (steering_base != NULL && getEntityAddy != NULL && steering_control != NULL && steering_parked != NULL) {
		...
		SteeringParked = new MemPatcher(steering_parked, 7, bytes);
		
		SteeringParked->Patch();
